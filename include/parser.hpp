...
private:
    ...
    _macros[name] = MacroCommand::New(canvas, name, Coordinate(0, 0), cmds);
}
    ...
    std::shared_ptr<Command> GetMacroCommand(std::string& cmd, const Coordinate& offset) {
        return _macros[cmd]->Copy(offset);
    }

    std::map<std::string, std::shared_ptr<MacroCommand>> _macros;
};
